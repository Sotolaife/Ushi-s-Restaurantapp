...
const sb = {
  ...
  authHeaders: (token) => ({ "apikey": SUPABASE_ANON, "Authorization": `Bearer ${token}`, "Content-Type": "application/json", "Prefer": "return=representation" }),
  async query(path, options = {}) { const res = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, { headers: this.headers, ...options }); if (!res.ok) throw new Error(await res.text()); const text = await res.text(); return text ? JSON.parse(text) : []; },
  async authQuery(path, token, options = {}) { if (!token) throw new Error("No token"); const res = await fetch(`${SUPABASE_URL}/rest/v1/${path}`, { headers: this.authHeaders(token), ...options }); if (!res.ok) throw new Error(await res.text()); const text = await res.text(); return text ? JSON.parse(text) : []; },
  async signUp(email, password, fullName) { const res = await fetch(`${SUPABASE_URL}/auth/v1/signup`, { method: "POST", headers: this.headers, body: JSON.stringify({ email, password, data: { full_name: fullName } }) }); return res.json(); },
  async signIn(email, password) { const res = await fetch(`${SUPABASE_URL}/auth/v1/token?grant_type=password`, { method: "POST", headers: this.headers, body: JSON.stringify({ email, password }) }); return res.json(); },
  async signOut(token) { if (!token) return; await fetch(`${SUPABASE_URL}/auth/v1/logout`, { method: "POST", headers: this.authHeaders(token) }); },
  async createProfile(userId, email, fullName) { try { const res = await fetch(`${SUPABASE_URL}/rest/v1/profiles`, { method: "POST", headers: { "apikey": SUPABASE_ANON, "Authorization": `Bearer ${SUPABASE_ANON}`, "Content-Type": "application/json", "Prefer": "return=representation" }, body: JSON.stringify({ id: userId, email, full_name: fullName, wallet: 0, is_admin: false }) }); if (!res.ok) { const err = await res.text(); if (err.includes("duplicate")) return; throw new Error(err); } } catch(e) { if (!e.message?.includes("duplicate")) throw e; } },
};

async function trackLogin(email, name) { try { await fetch(GOOGLE_SCRIPT_URL, { method: 'POST', mode: 'no-cors', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, name, action: 'login' }) }); } catch(e) {} }
...
function Splash({ onDone }) {
  ...
  return React.createElement("div", { style: { position:"fixed",inset:0,background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",zIndex:9999 } },
    React.createElement("div", { style: { fontSize:64,fontWeight:900,color:C.white,fontFamily:"Georgia,serif",letterSpacing:"-2px" } }, React.createElement("span", { style: {color:C.gold} }, "U"), "shi's"),
...
function AuthScreen({ onAuth }) {
  const [mode, setMode] = useState("login"); const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [password, setPass] = useState(""); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  const submit = async (e) => { e?.preventDefault(); setError(""); if (!email.trim() || !password.trim()) return setError("Fill in all fields."); if (mode==="signup" && !name.trim()) return setError("Enter your name."); if (password.length < 6) return setError("Password: min 6 characters."); setLoading(true);
    try {
      if (mode === "signup") { const signUpRes = await sb.signUp(email.trim(), password, name.trim()); if (signUpRes.error) throw new Error(signUpRes.error.message); if (!signUpRes.user?.id) throw new Error("Signup failed."); await sb.createProfile(signUpRes.user.id, email.trim(), name.trim()); trackLogin(email.trim(), name.trim()); await new Promise(r => setTimeout(r, 800)); const signInRes = await sb.signIn(email.trim(), password); if (signInRes.error) { setMode("login"); throw new Error("Account created! Please log in."); } onAuth({ token: signInRes.access_token, user: signInRes.user }); }
      else { const res = await sb.signIn(email.trim(), password); if (res.error) throw new Error(res.error.message || "Invalid credentials"); if (!res.access_token) throw new Error("Login failed."); trackLogin(email.trim(), res.user?.user_metadata?.full_name || email.trim()); onAuth({ token: res.access_token, user: res.user }); }
  ...
  return React.createElement("div", { style: { minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px" } },
    React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:44,fontWeight:900,color:C.white,letterSpacing:"-2px",marginBottom:4 } }, React.createElement("span", { style: {color:C.gold} }, "U"), "shi's"),
    ...
    React.createElement("form", { onSubmit: submit, style: { width:"100%",maxWidth:360 } },
      ...
      React.createElement("div", { style: { textAlign:"center",fontFamily:"Courier New,monospace",fontSize:12,color:C.muted } }, mode==="login"?"New here? ":"Already have an account? ", React.createElement("span", { onClick:()=>{setMode(mode==="login"?"signup":"login");setError("");}, style: { color:C.gold,cursor:"pointer",textDecoration:"underline" } }, mode==="login"?"Create account":"Log in"))
    )
  );
}
  ...
  return React.createElement("div", { style: { background:C.card,borderRadius:16,border:`1.5px solid ${qty>0?C.gold:C.border}`,overflow:"hidden",boxShadow:qty>0?`0 0 20px ${C.gold}20`:"0 2px 8px rgba(0,0,0,0.3)" } },
    ...
    React.createElement("div", { style: { padding:"10px 12px 12px" } },
      ...
      React.createElement("div", { style: { display:"flex",justifyContent:"space-between",alignItems:"flex-end" } },
        React.createElement("div", null, React.createElement("div", { style: { fontSize:16,fontWeight:900,color:C.gold,fontFamily:"Georgia,serif" } }, fmt(item.price)), savings && React.createElement("span", { style: { fontSize:9,color:C.muted,textDecoration:"line-through" } }, fmt(item.old_price))),
...
function CartSheet({ cart, setCart, profile, token, onSuccess, onClose }) {
  const [payMethod, setPayMethod] = useState("pickup"); const [loading, setLoading] = useState(false); const [err, setErr] = useState(""); const total = cart.reduce((s,i)=>s+i.price*i.qty,0); const wb = Number(profile?.wallet||0);
  const upd = (id,d) => setCart(c=>c.map(i=>i.id===id?{...i,qty:Math.max(0,i.qty+d)}:i).filter(i=>i.qty>0));
  const placeOrder = async (ref=null) => { setLoading(true);setErr(""); try { const [order] = await sb.authQuery("orders",token,{method:"POST",body:JSON.stringify({user_id:profile.id,customer_name:profile.full_name||profile.email||"Customer",customer_email:profile.email||"",items:cart.map(i=>({id:i.id,name:i.name,price:i.price,qty:i.qty,emoji:i.emoji||"🍽️"})),total,payment_method:payMethod,payment_ref:ref||null,status:"pending"})}); if(payMethod==="wallet") await sb.authQuery(`profiles?id=eq.${profile.id}`,token,{method:"PATCH",body:JSON.stringify({wallet:wb-total})}); onSuccess(order.order_number); } catch(e) { setErr("Something went wrong."); } setLoading(false); };
  return React.createElement(React.Fragment, null,
    ...
    React.createElement("div", { style: { position:"fixed",bottom:0,left:0,right:0,background:C.surface,borderRadius:"24px 24px 0 0",zIndex:201,maxHeight:"80vh",display:"flex",flexDirection:"column",border:`1px solid ${C.border}`,borderBottom:"none",maxWidth:430,margin:"0 auto" } },
      ...
      React.createElement("div", { style: { padding:"8px 20px 12px",display:"flex",justifyContent:"space-between",alignItems:"center" } }, React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:18,fontWeight:800,color:C.white } }, "Your Order"), React.createElement("button", { onClick:onClose, style: { background:"none",border:"none",color:C.muted,fontSize:18,cursor:"pointer" } }, "✕")),
      React.createElement("div", { style: { overflowY:"auto",padding:"0 20px",flex:1 } },
        cart.map(item=>React.createElement("div", { key:item.id, style: { display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${C.border}` } },
          ...
          React.createElement("div", { style: { flex:1 } }, React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:13,fontWeight:700,color:C.white } }, item.name), React.createElement("div", { style: { fontSize:12,color:C.gold,fontWeight:700 } }, fmt(item.price))),
      ...
      React.createElement("div", { style: { padding:"14px 20px 24px",borderTop:`1px solid ${C.border}` } },
        ...
        React.createElement("div", { style: { display:"flex",justifyContent:"space-between",marginBottom:12,alignItems:"center" } }, React.createElement("span", { style: { fontFamily:"Courier New,monospace",fontSize:11,color:C.muted } }, "TOTAL"), React.createElement("span", { style: { fontFamily:"Georgia,serif",fontSize:22,fontWeight:900,color:C.gold } }, fmt(total))),
...
function HomeScreen({ session, profile, setProfile }) {
  const [menu, setMenu] = useState([]); const [cat, setCat] = useState("All"); const [cart, setCart] = useState([]); const [cartOpen, setCartOpen] = useState(false); const [orderNum, setOrderNum] = useState(null); const [ml, setMl] = useState(true);
  ...
  const cats = ["All",...new Set(menu.map(m=>m.category).filter(Boolean))]; const items = cat==="All"?menu:menu.filter(m=>m.category===cat); const cc = cart.reduce((s,i)=>s+i.qty,0); const ct = cart.reduce((s,i)=>s+i.price*i.qty,0); const gq = (id)=>cart.find(i=>i.id===id)?.qty||0;
  ...
  return React.createElement("div", { style: { background:C.bg,minHeight:"100vh",paddingBottom:100 } },
    React.createElement("div", { style: { padding:"20px 20px 0",display:"flex",justifyContent:"space-between",alignItems:"center" } },
      React.createElement("div", null, React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:28,fontWeight:900,color:C.white,letterSpacing:"-0.5px" } }, React.createElement("span", { style: {color:C.gold} }, "U"), "shi's"), React.createElement("div", { style: { fontSize:10,color:C.muted,fontFamily:"Courier New,monospace",letterSpacing:"2px",marginTop:2 } }, "UNILORIN CAMPUS")),
      React.createElement("div", { style: { background:C.surface,border:`1px solid ${C.border}`,borderRadius:14,padding:"8px 14px",display:"flex",alignItems:"center",gap:8 } }, React.createElement("span", null, "💳"), React.createElement("div", null, React.createElement("div", { style: { fontSize:8,color:C.muted,fontFamily:"Courier New,monospace" } }, "WALLET"), React.createElement("div", { style: { fontSize:13,fontWeight:800,color:C.white,fontFamily:"Georgia,serif" } }, fmt(Number(profile?.wallet||0)))))
    ),
    React.createElement("div", { style: { padding:"16px 20px 14px" } }, React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:14,fontWeight:700,color:C.muted } }, "Hey "+(profile?.full_name?.split(" ")[0]||"there")+" 👋 — what are you feeling today?")),
    React.createElement("div", { style: { marginBottom:16 } }, React.createElement("div", { style: { display:"flex",gap:8,padding:"0 20px",overflowX:"auto",scrollbarWidth:"none" } }, cats.map(c=>React.createElement("button", { key:c, onClick:()=>setCat(c), style: { padding:"8px 18px",borderRadius:20,border:"none",background:cat===c?C.gold:C.card,color:cat===c?C.bg:C.muted,fontFamily:"Georgia,serif",fontSize:13,fontWeight:cat===c?800:500,cursor:"pointer",whiteSpace:"nowrap",flexShrink:0 } }, c)))),
    ...
    cc>0&&React.createElement("div", { onClick:()=>setCartOpen(true), style: { position:"fixed",bottom:80,left:20,right:20,maxWidth:390,margin:"0 auto",background:C.gold,borderRadius:16,padding:"14px 20px",zIndex:150,display:"flex",justifyContent:"space-between",alignItems:"center",boxShadow:`0 8px 32px ${C.gold}55`,cursor:"pointer" } },
      React.createElement("div", { style: { display:"flex",alignItems:"center",gap:10 } }, React.createElement("div", { style: { background:C.bg,color:C.gold,borderRadius:8,width:28,height:28,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900 } }, cc), React.createElement("span", { style: { fontFamily:"Georgia,serif",fontWeight:800,fontSize:14,color:C.bg } }, "View Order")),
...
function WalletScreen({ session, profile, setProfile }) {
  ...
  return React.createElement("div", { style: { background:C.bg,minHeight:"100vh",paddingBottom:90,color:C.white } },
    ...
    React.createElement("div", { style: { margin:"0 20px 24px",background:`${C.gold}15`,border:`1px solid ${C.gold}33`,borderRadius:18,padding:22 } }, React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:10,color:C.gold,letterSpacing:"2px",marginBottom:8 } }, "CURRENT BALANCE"), React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:40,fontWeight:900 } }, fmt(Number(profile?.wallet||0)))),
    ...
    React.createElement("div", { style: { padding:"0 20px",marginBottom:16 } }, React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:10,color:C.muted,letterSpacing:"2px",marginBottom:12 } }, "QUICK AMOUNTS"), React.createElement("div", { style: { display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 } }, [1000,2000,5000,10000].map(p=>React.createElement("button", { key:p, onClick:()=>topUp(p), style: { padding:"14px",background:C.card,border:`1px solid ${C.border}`,borderRadius:14,fontFamily:"Georgia,serif",fontSize:16,fontWeight:800,color:C.white,cursor:"pointer" } }, fmt(p))))),
    React.createElement("div", { style: { padding:"0 20px" } }, React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:10,color:C.muted,letterSpacing:"2px",marginBottom:12 } }, "CUSTOM AMOUNT"), React.createElement("div", { style: { display:"flex",gap:10 } }, React.createElement("input", { type:"number", placeholder:"Enter amount", value:amount, onChange:e=>setAmount(e.target.value), style: { flex:1,background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"16px",color:C.white,fontSize:15,fontFamily:"Georgia,serif",outline:"none" } }), React.createElement("button", { onClick:()=>topUp(Number(amount)), disabled:loading, style: { background:loading?C.dim:C.gold,border:"none",borderRadius:14,padding:"16px 24px",color:C.bg,fontFamily:"Georgia,serif",fontSize:16,fontWeight:800,cursor:"pointer" } }, loading?"…":"Pay")))
  );
}
  ...
  return React.createElement("div", { style: { background:C.bg,minHeight:"100vh",paddingBottom:90,color:C.white } },
    ...
    React.createElement("div", { style: { padding:"0 20px",display:"flex",flexDirection:"column",gap:12 } },
      ...
      orders.map(o=>React.createElement("div", { key:o.id, style: { background:C.card,borderRadius:16,padding:16,border:`1px solid ${C.border}`,display:"flex",gap:12,alignItems:"center" } },
        ...
        React.createElement("div", { style: {flex:1} }, React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:9,color:C.muted } }, "#"+String(o.order_number).padStart(4,"0")), React.createElement("div", { style: { fontFamily:"Georgia,serif",fontWeight:800,fontSize:13,color:C.white,marginTop:2 } }, o.items?.map(i=>i.name).join(", ")), React.createElement("div", { style: { fontSize:13,color:C.gold,fontWeight:700 } }, fmt(o.total))),
...
function ProfileScreen({ profile, onSignOut }) {
  return React.createElement("div", { style: { background:C.bg,minHeight:"100vh",paddingBottom:90,color:C.white } },
    ...
    React.createElement("div", { style: { margin:"18px 20px",background:C.card,borderRadius:18,padding:18,border:`1px solid ${C.border}`,display:"flex",gap:14,alignItems:"center" } }, React.createElement("div", { style: { width:52,height:52,borderRadius:16,background:`linear-gradient(135deg,${C.gold}88,${C.gold}44)`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24 } }, "👤"), React.createElement("div", null, React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:16,fontWeight:800 } }, profile?.full_name||"User"), React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:10,color:C.muted,marginTop:3 } }, profile?.email||"—"))),
    React.createElement("div", { style: { margin:"0 20px 18px",background:`${C.gold}15`,border:`1px solid ${C.gold}33`,borderRadius:18,padding:18 } }, React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:10,color:C.gold,letterSpacing:"2px",marginBottom:6 } }, "WALLET BALANCE"), React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:34,fontWeight:900 } }, fmt(Number(profile?.wallet||0)))),
    React.createElement("div", { style: { padding:"0 20px" } },
      ...
      React.createElement("div", { onClick:onSignOut, style: { background:`${C.red}08`,borderRadius:14,padding:"14px 18px",border:`1px solid ${C.red}22`,cursor:"pointer" } }, "🚪 ", React.createElement("span", { style: {color:C.red,fontWeight:700} }, "Log Out"))
    )
  );
}
...
function AdminScreen({ session }) {
  ...
  return React.createElement("div", { style: { background:C.bg,minHeight:"100vh",paddingBottom:90,color:C.white } },
    ...
    orders.map(o=>React.createElement("div", { key:o.id, style: { margin:"0 20px 12px",background:C.card,borderRadius:16,padding:16,border:`1px solid ${C.border}` } },
      React.createElement("div", { style: { display:"flex",justifyContent:"space-between",marginBottom:8 } }, React.createElement("span", { style: { fontFamily:"Courier New,monospace",fontSize:10,color:C.muted } }, "#"+String(o.order_number).padStart(4,"0")), React.createElement("span", { style: { fontSize:10,color:SC[o.status]||C.muted,textTransform:"capitalize" } }, o.status?.replace(/_/g," "))),
      ...
      React.createElement("div", { style: { display:"flex",justifyContent:"space-between",alignItems:"center" } }, React.createElement("span", { style: { fontFamily:"Georgia,serif",color:C.gold,fontWeight:800 } }, fmt(o.total)), React.createElement("div", { style: { display:"flex",gap:6 } }, ["pending","preparing","ready","picked_up"].filter(s=>s!==o.status).slice(0,2).map(s=>React.createElement("button", { key:s, onClick:()=>updateStatus(o.id,s), style: { padding:"5px 10px",borderRadius:8,border:`1px solid ${SC[s]||C.border}`,background:"transparent",color:SC[s]||C.muted,fontFamily:"Courier New,monospace",fontSize:9,cursor:"pointer",textTransform:"capitalize" } }, "→ "+s.replace(/_/g," ")))))
    ))
  );
}
...
function AdminGate({ onUnlock }) {
  ...
  return React.createElement("div", { style: { minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:40,color:C.white } },
    React.createElement("div", { style: { fontSize:36,marginBottom:16 } }, "🔐"), React.createElement("div", { style: { fontFamily:"Georgia,serif",fontSize:20,fontWeight:900,marginBottom:8 } }, "Admin Access"), React.createElement("div", { style: { fontFamily:"Courier New,monospace",fontSize:11,color:C.muted,marginBottom:32 } }, "Enter PIN"),
    React.createElement("div", { style: { display:"flex",gap:14,marginBottom:32 } }, [0,1,2,3].map(i=>React.createElement("div", { key:i, style: { width:16,height:16,borderRadius:"50%",background:pin.length>i?(err?C.red:C.gold):C.dim } }))),
    React.createElement("div", { style: { display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,maxWidth:260,width:"100%" } }, [1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((d,i)=>React.createElement("button", { key:i, onClick:()=>d==="⌫"?setPin(p=>p.slice(0,-1)):d!==""&&check(pin+String(d)), disabled:d==="", style: { padding:"18px",borderRadius:14,border:`1px solid ${C.border}`,background:C.card,color:C.white,fontFamily:"Georgia,serif",fontSize:22,fontWeight:800,cursor:d===""?"default":"pointer" } }, d)))
  );
}
